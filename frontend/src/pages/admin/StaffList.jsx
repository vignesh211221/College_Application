...
const StaffList = () => {
  ...
  const handleAssignHOD = async (id) => {
    if (window.confirm("Are you sure you want to assign HOD role?")) {
      try {
        await assignHODRole(id);
        toast.success("HOD role assigned");
        fetchStaff();
      } catch (error) {
        toast.error("Failed to assign HOD role");
      }
    }
  };

  const handleRemoveHOD = async (id) => {
    if (window.confirm("Are you sure you want to remove HOD role?")) {
      try {
        await removeHODRole(id);
  ...
  const handleAssignPlacementOfficer = async (id) => {
    if (window.confirm("Are you sure you want to assign Placement Officer role?")) {
      try {
        await assignPlacementOfficer(id);
        toast.success("Placement Officer role assigned");
        fetchStaff(); 
      } catch (error) {
        toast.error("Failed to assign Placement Officer role");
      }
    }
  };
  
  const handleRemovePlacementOfficer = async (id) => {
    if (window.confirm("Are you sure you want to remove Placement Officer role?")) {
      try {
        await removePlacementOfficer(id);
        toast.success("Placement Officer role removed");
        fetchStaff(); 
      } catch (error) {
        toast.error("Failed to remove Placement Officer role");
      }
    }
  };
  ...
  return (
    <div>
      ...
      <br></br>
      <br></br>      <div className="student-list-container">
              ...
              <tbody>
                {staffList.map((staff) => (
                  <tr key={staff._id}>
                    ...
                    <td>
                      <div className="action-buttons">
                        ...
                        <button
                          className="edit-btn"
                          onClick={() => handleAssignHOD(staff._id)}
                        >
                          Assign HOD
                        </button>
                        {staff.user?.role === "hod" && (
                          <button
                            className="delete-btn"
                            onClick={() => handleRemoveHOD(staff._id)}
                          >
                            Remove HOD
                          </button>
                        )}
                        {staff.user?.role !== "placementOfficer" && (
                    <button
                      className="edit-btn"
                      onClick={() => handleAssignPlacementOfficer(staff._id)}
                    >
                  ...
                  {staff.user?.role === "placementOfficer" && (
                    <button
                      className="delete-btn"
                      onClick={() => handleRemovePlacementOfficer(staff._id)}
                    >
                      Remove Placement Officer
                    </button>
                  )}
                      </div>
                    </td>
